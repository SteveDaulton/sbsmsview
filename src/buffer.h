...
class RingBuffer {
 public:
  ...
  T &get(long k) const; //reference indexed from readpos
  T const &read(long k) const; //const reference at readpos
  T const &read() const; //const reference indexed from 0
  long nReadable() const;
  void advance(long n);
  void clear();
  long readPos;
  long writePos;
...
template <class T>
T &RingBuffer<T> :: get(long k) const
{
  return buf[readPos+k];
}

template <class T>
T const &RingBuffer<T> :: read(long k) const
{
  return buf[k];
}

template <class T>
T const &RingBuffer<T> :: read() const
{
  return buf[readPos];
}

template <class T>
long RingBuffer<T> :: nReadable() const
{
...
template<class T>
long ArrayRingBuffer<T> :: read(T *outBuf, long n)
{
