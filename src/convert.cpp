...
{  
  ...
  if(bAnalyze) {
    ...
    sbsms = new SBSMS(channels,&quality,true,false);
...
{
  AudioDecoder *decoder = import(filenameIn);
  if(!decoder) {
    printf("File: %s cannot be opened\n",filenameIn);
    exit(-1);
  }
  SBSMSQuality quality(&SBSMSQualityStandard);
  Slide rateSlide(SlideLinearInputRate,1.0f,1.0f);
  Slide pitchSlide(SlideLinearOutputRate,1.0f,1.0f);
  float srOut = 44100.0f;
  float srIn = decoder->getSampleRate();
  int channels = decoder->getChannels();
  SampleCountType samplesIn = decoder->getFrames();

  SampleCountType samplesToInput = (SampleCountType) (samplesIn*(float)srOut/(float)srIn);
  float pitch = (srOut == srIn?1.0f:(float)srOut / (float)srIn);
  SBSMSInterface *iface = new SBSMSInterfaceDecoder(&rateSlide,&pitchSlide,false,
                                                    channels,samplesToInput,0,&quality,decoder,pitch);
  SampleCountType samplesToOutput = iface->getSamplesToOutput();

  SBSMS *sbsms = new SBSMS(channels,&quality,false,true);
  sbsms->setTotalSamples(samplesToInput);

  SampleCountType tot = 0;
  SampleCountType lastTot;
	SampleCountType n;
  do {
    n = sbsms->renderFrame(iface);
    lastTot = tot;
    tot += n;
  } while(n && lastTot < samplesToInput);
