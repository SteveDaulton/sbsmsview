...
GrainAllocator :: GrainAllocator(int N, int N2, int type)
{
  ...
  for(int k=0;k<N2;k++) {
    if(type == hann) {
      w[k+k1] = 0.5f*(1.0f - cos((float)k/(float)(N2-1)*TWOPI));
    } else if(type == hannpoisson) {
      w[k+k1] = 0.5f*(1.0f - cos((float)k/(float)(N2-1)*TWOPI)) * exp(-1.0f*fabs((float)(N2-1-k-k))/(float)(N2-1));
    } else if(type == hannpoissonT) {
      w[k+k1] = (k-N2/2)*0.5f*(1.0f - cos((float)k/(float)(N2-1)*TWOPI)) * exp(-1.0f*fabs((float)(N2-1-k-k))/(float)(N2-1));
    }
  }
